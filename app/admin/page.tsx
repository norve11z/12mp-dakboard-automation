    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="sticky top-0 z-10 bg-gray-950 border-b border-gray-800 py-3 mb-6 flex items-center gap-6 text-sm">
        ...
        <div><span className="text-gray-500">Last import:</span>{" "}
          <strong>{stats?.lastImport ? new Date(stats.lastImport + "Z").toLocaleString() : "never"}</strong>
        </div>
...
function AddGameModal({ onClose, onDone, defaultDate }: { onClose: () => void; onDone: () => void; defaultDate: string; }) {
  ...
  const [crewText, setCrewText] = useState("Producer, Sam Smith\nDirector, Alex Jones\nCamera 1, Jamie Lee");
  ...
  const submit = async () => {
    const crew = crewText
      ...
      .filter(Boolean)
      .map(l => {
        const [position, ...rest] = l.split(",");
        return { position: position.trim(), name: rest.join(",").trim() };
      })
    ...
    const res = await fetch("/api/add-game", {
      ...
      body: JSON.stringify({ sport, game_date, display_type, crew_call, kickoff, opponent, crew }),
    });
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-6 w-full max-w-2xl m-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm mb-3">
          ...
          <label className="flex flex-col gap-1">
            <span className="text-gray-400">Crew Call (HH:MM)</span>
            <input type="time" value={crew_call} onChange={e => setCrewCall(e.target.value)}
              className="bg-gray-800 px-2 py-1 rounded" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-gray-400">Kickoff / Game Time (HH:MM)</span>
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-gray-400">Crew (one per line, format: <code>Position, Name</code>)</span>
          <textarea value={crewText} onChange={e => setCrewText(e.target.value)}
            rows={8} className="bg-gray-800 px-2 py-1 rounded font-mono text-xs" />
        </label>
